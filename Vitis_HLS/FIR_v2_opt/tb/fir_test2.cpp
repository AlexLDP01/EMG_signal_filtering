...
int main() {
	...
	// testbench process:
		...
		// RNC: To avoid left shift overflow
		int64_t aux = 1;
		aux <<= (WL_acum - IL_acum);
		...
		for (int k = 0; k < samples; k++)
		{
			...
			reads_64 = fread (&buffer_line_Ro, sizeof(int64_t), 1, fpRo);

			EMG[k] 	   = ((float)buffer_line_Ri) / (1 << (WL_data - IL_data));
			envelop[k] = ((double)buffer_line_Ro) / (aux);
			...
			if( abs(diff) > 1e-3){
				cout << "Error" << endl;
				cout << "Input  = " << EMG[k].to_float() << endl;
